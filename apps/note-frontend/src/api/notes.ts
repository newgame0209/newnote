...
export const createNote = async (data: CreateNoteData): Promise<Note> => {
  try {
    const response = await noteApiClient.post('/notes', data);
...
export const fetchNote = async (id: string): Promise<Note> => {
  try {
    const response = await noteApiClient.get(`/notes/${id}`);
    return response.data;
  } catch (error) {
...
export const updateNote = async (id: string, data: Partial<CreateNoteData>): Promise<Note> => {
  try {
    const response = await noteApiClient.put(`/notes/${id}`, data);
    return response.data;
  } catch (error) {
...
export const deleteNote = async (id: string): Promise<void> => {
  try {
    await noteApiClient.delete(`/notes/${id}`);
  } catch (error) {
...
export const fetchNotes = async (): Promise<Note[]> => {
  try {
    const response = await noteApiClient.get('/notes');
    return response.data;
  } catch (error) {
...
export const updatePage = async (noteId: string | number, pageNumber: number, content: string): Promise<any> => {
  try {
    const response = await noteApiClient.put(`/notes/${noteId}/pages/${pageNumber}`, { content });
    return response.data;
  } catch (error) {
...
export const getPage = async (noteId: string | number, pageNumber: number): Promise<any> => {
  try {
    const response = await noteApiClient.get(`/notes/${noteId}/pages/${pageNumber}`);
    return response.data;
  } catch (error) {
...
export const executeOCR = async (noteId: number, pageNumber: number, imageData: string): Promise<string> => {
  try {
    const response = await noteApiClient.post<{ text: string }>(`/notes/${noteId}/pages/${pageNumber}/ocr`, { image: imageData });
...
export const synthesizeSpeech = async (
  ...
  try {
    ...
    const response = await noteApiClient.post('/tts', {
      text,
      voice: {
        language_code: 'ja-JP',
